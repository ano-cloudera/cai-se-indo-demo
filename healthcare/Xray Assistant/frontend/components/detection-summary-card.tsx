  ...
  return (
    <PanelCard className="p-6">
      <PanelHeader
        ...
        subtitle="A concise overview of the latest analysis result."
        icon={
        ...
        <>
          <div className="mt-5 space-y-2 text-sm">
            <div className="flex items-baseline justify-between gap-3">
              <span className="text-xs font-semibold uppercase tracking-[0.12em] text-[var(--color-ink-subtle)]">Case ID</span>
              <span className="break-all text-right font-semibold text-[var(--color-ink-strong)]">{result.case_id}</span>
            </div>
            <div className="flex items-baseline justify-between gap-3">
              <span className="text-xs font-semibold uppercase tracking-[0.12em] text-[var(--color-ink-subtle)]">Model</span>
              <span className="break-all text-right font-semibold text-[var(--color-ink-strong)]">{result.model_info.name}</span>
            </div>
          </div>

          <div className="mt-4 rounded-[18px] border border-[var(--color-border-soft)] bg-[var(--color-surface-muted)] p-4">
            <p className="text-xs font-semibold uppercase tracking-[0.12em] text-[var(--color-ink-subtle)]">Summary</p>
            <p className="mt-2 text-sm leading-6 text-[var(--color-ink-muted)]">{result.summary}</p>
          </div>

          <div className="mt-4 flex flex-wrap gap-2 text-xs text-[var(--color-ink-subtle)]">
            <span className="rounded-full border border-[var(--color-border-soft)] bg-[var(--color-surface-muted)] px-3 py-1">
