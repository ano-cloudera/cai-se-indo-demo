...
def resolve_frontend_dir() -> Path:
    ...
    candidates: list[Path] = [
        cwd / "cai-se-indo-demo" / "healthcare" / "Xray Assistant" / "frontend",
        cwd / "healthcare" / "Xray Assistant" / "frontend",
        cwd / "Xray Assistant" / "frontend",
        cwd / "frontend",
    ]
    ...
    try:
        for entry in sorted(cwd.iterdir()):
            if entry.is_dir():
                candidates.append(entry / "healthcare" / "Xray Assistant" / "frontend")
                candidates.append(entry / "Xray Assistant" / "frontend")
    ...
    for candidate in candidates:
        exists = (candidate / "package.json").exists()
        logging.info("  %s -> package.json exists=%s", candidate, exists)
        if exists:
            return candidate

    raise SystemExit(
        f"Could not find Xray Assist frontend directory with package.json. cwd={cwd}. Check logs above."
    )
...
def resolve_binary(name: str) -> str:
    ...
    raise SystemExit(
        f"Could not find '{name}' in PATH. Make sure Node.js and npm are available in the CAI runtime."
    )
...
def main() -> None:
    ...
    logging.info("Starting Xray Assist frontend")
